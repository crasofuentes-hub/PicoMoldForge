...
public sealed class GeneratorRunManifestSha256Tests
{
    [Fact]
    public void Run_WithGenerateAll_WritesSha256ForEveryArtifact()
    {
        var tempDir = Path.Combine(Path.GetTempPath(), $"picomoldforge-generator-run-manifest-sha256-{Guid.NewGuid():N}");
        ...
        try
        {
            ...
            Assert.True(artifacts.GetArrayLength() >= 10);
            ...
            foreach (var artifact in artifacts.EnumerateArray())
            {
                ...
                Assert.False(string.IsNullOrWhiteSpace(sha256));
                Assert.Matches("^[a-f0-9]{64}$", sha256);
                Assert.Equal(ComputeSha256(artifactPath!), sha256);
            }
        }
        finally
        {
        ...
        string outputDirectory)
    {
        ...
        {
          "projectName": "Run Manifest Sha256 Test",
          "inputPath": "{{inputPath}}",
    ...
    private static void WriteBinaryBoxStl(string path, float sizeX, float sizeY, float sizeZ)
    {
        ...
        var headerText = "PicoMoldForge run manifest sha256 binary box STL test"u8.ToArray();
