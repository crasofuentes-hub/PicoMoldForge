...
public sealed class GeneratorCleanOutputOptionTests
{
    [Fact]
    public void Run_WithGenerateAllAndCleanOutput_RemovesStaleOutputFiles()
    {
        var tempDir = Path.Combine(Path.GetTempPath(), $"picomoldforge-generator-clean-output-{Guid.NewGuid():N}");
        ...
        var staleFile = Path.Combine(outputDirectory, "stale.txt");
    ...
    [Fact]
    public void Run_WithGenerateAllWithoutCleanOutput_PreservesStaleOutputFiles()
    {
        var tempDir = Path.Combine(Path.GetTempPath(), $"picomoldforge-generator-preserve-output-{Guid.NewGuid():N}");
        Directory.CreateDirectory(tempDir);
    ...
    [Fact]
    public void Run_WithHelp_IncludesCleanOutputOption()
    {
        using var output = new StringWriter();
        using var error = new StringWriter();
        ...
        Assert.Equal(0, exitCode);
        Assert.Contains("--clean-output", output.ToString());
        Assert.Equal(string.Empty, error.ToString());
    }
    ...
    private static void WriteBinaryBoxStl(string path, float sizeX, float sizeY, float sizeZ)
    {
        ...
        var headerText = "PicoMoldForge clean output binary box STL test"u8.ToArray();
